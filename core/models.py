...
class GameSystem(models.Model):
    ...
    version = models.CharField(
        max_length=20, blank=True, help_text="Версия системы, например, 1.0"
    )
    slug = models.SlugField(
        unique=True, help_text="Короткое имя для URL, например, 'daggerheart'"
    )
...
class FeatureSet(models.Model):
    ...
    system = models.ForeignKey(
        GameSystem, on_delete=models.CASCADE, related_name="feature_sets"
    )
    set_type = models.CharField(
        max_length=50, blank=True, help_text="Тип набора, например 'Domain'"
    )
...
class Feature(models.Model):
    ...
    system = models.ForeignKey(
        GameSystem, on_delete=models.CASCADE, related_name="features"
    )
...
class TraitCategory(models.Model):
    ...
    name = models.CharField(
        max_length=100, help_text="Тип характеристики, например, 'Класс' или 'Раса'"
    )
    system = models.ForeignKey(
        GameSystem, on_delete=models.CASCADE, related_name="trait_categories"
    )
...
class CharacterTrait(models.Model):
    ...
    system = models.ForeignKey(
        GameSystem, on_delete=models.CASCADE, related_name="character_traits"
    )
...
class DamageType(models.Model):
    ...
    system = models.ForeignKey(
        GameSystem, on_delete=models.CASCADE, related_name="damage_types"
    )
...
class EquipmentTemplate(models.Model):
    ...
    description = models.TextField(blank=True)
    system = models.ForeignKey(
        GameSystem, on_delete=models.CASCADE, related_name="equipment_templates"
    )
